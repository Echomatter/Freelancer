...
Open project → **Browse folders…** selects a folder, including local drive roots. **Next** checks for local Codex history. New projects offer **Bring your chats along?** before file and conversation indexing. Select conversations or **Skip import**. Nothing is selected by default. Already registered projects open normally.

This is a one-time, one-way snapshot. Only entries whose recorded working directory matches the exact resolved project folder are eligible. Sibling folders, nested folders and other worktrees are not combined by remote URL or name. The transcript's session ID and directory must also match. Archived source chats are offered; their imported copies are available, with original archive status retained as provenance.
