...
The composer has one dynamic action: Send when idle with content, Stop when a response is running and the text box is empty, and Queue / Delegate when text is entered during a response. Stop uses a circular icon so it does not look like a checkbox. Opening or dismissing Queue / Delegate never aborts a response. Shift+Enter and IME composition retain their normal editing behavior. Local attachments use native OpenCode file parts on a normal send. They stay in the browser until sent, are limited to four files (4 MB each, 6 MB total), and are not included in saved text drafts or Queue / Delegate. During a response, the action stays Stop if only files are attached; typed text opens Queue / Delegate and leaves those files in the composer for a later send.
