...
try {
  ...
  const workerCard = page.locator(".activity-detail-toggle").first();
  assert.equal(await workerCard.getAttribute("aria-expanded"), "false");
  assert.equal(await page.getByText("Browser worker job", { exact: true }).count(), 0);
  await workerCard.click();
  assert.equal(await workerCard.getAttribute("aria-expanded"), "true");
  assert.equal(await page.getByText("Browser worker job", { exact: true }).count(), 1);
  fixture.setActivity("completed");
  await page.waitForFunction(() => document.querySelector(".activity-detail-toggle")?.getAttribute("aria-expanded") === "false");
  assert.equal(await page.getByText("Browser worker job", { exact: true }).count(), 0);
  await workerCard.click();
  assert.equal(await workerCard.getAttribute("aria-expanded"), "true");
  assert.equal(await page.getByText("Browser worker job", { exact: true }).count(), 1);
  report("Details title and worker auto-collapse on completion");
  ...
  await failSave({ error: "Theme save failed" }); await theme.selectOption("light");
  ...
  await theme.selectOption("light"); await page.waitForFunction(() => document.documentElement.dataset.theme === "light");
  assert.equal((await fixture.store.read("settings")).appearance.theme, "light");
  await theme.selectOption("dark"); await page.waitForFunction(() => document.documentElement.dataset.theme === "dark");
  await load();
